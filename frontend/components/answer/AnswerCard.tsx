...
export const AnswerCard = ({ result }: { result: QueryResult }) => {
  return (
    <motion.div 
      ...
      className="bg-white border border-bg-border rounded-lg mb-10 overflow-hidden shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.08)] transition-all duration-700"
    >
      {/* Precision Header */}
      ...
      <div className="p-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-bg-border border border-bg-border rounded-sm overflow-hidden mb-12 shadow-sm">
          <div className="bg-white p-8 group">
            <span className="mono text-[9px] uppercase text-text-tertiary block mb-3 tracking-[0.2em] group-hover:text-accent-main transition-colors">{result.metricName}</span>
          ...
          <div className="bg-white p-8">
            <span className="mono text-[9px] uppercase text-text-tertiary block mb-3 tracking-[0.2em]">Contextual Variance</span>
            <div className="flex items-center gap-3">
              <span className={`text-2xl font-light italic ${result.changeDirection === 'up' ? 'text-zinc-900' : 'text-zinc-500'}`}>
          ...
          <div className="bg-white p-8 flex flex-col justify-center">
            <span className="mono text-[9px] uppercase text-text-tertiary block mb-3 tracking-[0.2em]">Process latency</span>
            <span className="text-xl font-light mono text-text-secondary">{result.durationMs}ms</span>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-start">
          <div className="flex-1 space-y-8">
             <div className="flex items-center gap-3 text-text-tertiary italic">
                ...
                <span className="text-sm font-medium">Analytical Interpretation</span>
             </div>
             ...
             <div className="pt-6 border-t border-bg-border flex items-center gap-3 text-text-tertiary">
                <Info size={14} />
                <span className="text-[10px] mono uppercase tracking-wider">{result.sourceSummary}</span>
             </div>
          </div>
        ...
        <div className="flex items-center gap-8">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center justify-between">
                <span className="mono text-[8px] uppercase font-bold text-text-tertiary">Precision Index</span>
                <span className="mono text-[8px] uppercase font-bold text-accent-main">{Math.round(result.confidence * 100)}%</span>
            </div>
          </div>
        </div>
        
        <div className="flex items-center gap-4">
            <span className="text-[10px] mono text-text-tertiary uppercase">Local Intelligence Active</span>

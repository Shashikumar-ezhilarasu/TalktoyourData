...
export class StatisticalEngine {
  ...
  computeComparison(
    ...
    const groupAValues = this.extractNumeric(
      data,
      metricCol,
      dimensionCol,
      groupA,
    );
    const groupBValues = this.extractNumeric(
      data,
      metricCol,
      dimensionCol,
      groupB,
    ...
    // Welch's t-test
    const tTest =
      groupAValues.length > 1 && groupBValues.length > 1
    ...
    const cohensD =
      (ss.mean(groupBValues) - ss.mean(groupAValues)) / (pooledStd || 1);

    return {
      groupA: {
        label: groupA,
        sum: sumA,
        mean: ss.mean(groupAValues),
        count: groupAValues.length,
      },
      groupB: {
        label: groupB,
        sum: sumB,
        mean: ss.mean(groupBValues),
  ...
  computeBreakdown(
    ...
    const totals = Array.from(groups.entries())
      .map(([dim, vals]) => ({
        dimension: dim,
        value: ss.sum(vals),
        count: vals.length,
    ...
    const paretoThreshold =
      withShares.findIndex((t) => t.cumulativeShare >= 80) + 1;
    ...
    return {
      ...
      paretoSegmentCount:
        paretoThreshold > 0 ? paretoThreshold : withShares.length,
    };
  }
  ...
  computeAnomalies(data: any[], metricCol: string): AnomalyStats {
    const values = data
      ...
      .filter((v) => !isNaN(v));
    if (values.length === 0)
      throw new Error("No numeric data for anomaly detection");
    ...
    const iqrAnomalies = values.filter(
